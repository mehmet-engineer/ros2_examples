...
class PublisherClass : public rclcpp::Node
{
  ...
  public:
    ...
    PublisherClass() : Node("publisher_executor_node")
    {
      ...
      timer = this->create_wall_timer(timer_period, std::bind(&PublisherClass::timer_callback, this), cb_group);
...
class SubscriberClass : public rclcpp::Node
{
  ...
  public:
    ...
    { 
      ...
      cb_options.callback_group = my_callback_group;

      string_subscription = this->create_subscription<std_msgs::msg::String>("/publisher_executor", 10, 
                        std::bind(&SubscriberClass::subscriber_callback_1, this, std::placeholders::_1), cb_options);
      other_subscription = this->create_subscription<std_msgs::msg::String>("/publisher_executor", 10, 
                        std::bind(&SubscriberClass::subscriber_callback_2, this, std::placeholders::_1), cb_options);
      std::this_thread::sleep_for(std::chrono::seconds(1));
    }
